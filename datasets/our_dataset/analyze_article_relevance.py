...
def load_api_config():
    ...
    auth_file = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "auth.yaml"
    )
    if not os.path.exists(auth_file):
        raise FileNotFoundError(f"配置文件 {auth_file} 不存在")

    with open(auth_file, "r", encoding="utf-8") as f:
        config = yaml.safe_load(f)

    # 使用grok配置
    if "grok" not in config:
        raise ValueError("auth.yaml中未找到grok配置")
...
# 输入和输出文件路径
INPUT_FILE = "datasets/our_dataset/sciencealert_articles_with_wikipedia.json"
OUTPUT_FILE = "datasets/our_dataset/science_alert_analyzed_articles.json"
...
"""
    ...
    return {"original_data": data, "analysis": response_json}


def process_article_pair(
    data: Dict[str, Any], index: int, total: int
) -> Tuple[int, Dict[str, Any]]:
    """处理单对文章"""
    print(
        f"处理文章对 {index+1}/{total}: {data.get('popsci_article', {}).get('title', '未知标题')}"
    )
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=500) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_index):
            ...
            print(
                f"已完成 {completed_count}/{len(data_list)} ({completed_count/len(data_list)*100:.1f}%), "
    ...
    if scores:
        ...
        print(
            f"高关联性(≥7分)文章对数量: {high_score_count}/{len(scores)} ({high_score_count/len(scores)*100:.1f}%)"
        )
